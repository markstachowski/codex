...
async fn thread_id_generator_applies_to_roots_children_and_forks() {
    ...
    assert_eq!(
        fork.session_configured.session_id, root.session_configured.session_id,
        "forked subagent must share its root session identity"
    );
    assert_eq!(
        fork.session_configured.parent_thread_id,
        Some(root.thread_id),
        "forked subagent must retain its direct parent thread identity"
    );
    assert_eq!(
        fork.session_configured.forked_from_id,
        Some(root.thread_id),
        "forked subagent must retain its copied-history source"
    );
