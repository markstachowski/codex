...
impl Session {
    ...
    async fn capture_step_context_inner(
        ...
        if matches!(
            ...
            let lane = crate::config::locked_model_policy_lane()
                .map_err(|err| CodexErr::InvalidRequest(err.to_string()))?;
            let child_service_tier = self
                .services
                .agent_control
                .resolve_child_service_tier(lane, settings.selected().service_tier.as_deref())
                .map_err(|err| CodexErr::InvalidRequest(err.to_string()))?;
    ...
    fn enforce_server_model_match(
        ...
        let requested_model = step_context.settings.model_info.slug.as_str();
        ...
        let session_scope = if step_context.turn.session_source.is_non_root_agent() {
            "non-root"
        } else {
            "root"
        };
