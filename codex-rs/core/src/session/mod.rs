...
fn apply_locked_new_root_inference_defaults_for_lane(
    ...
    let Some((model, effort)) =
        locked_new_root_inference_defaults_for_lane(conversation_history, session_source, lane)
    else {
        return;
    };
    ...
    let lane = lane.expect("managed root defaults require a model policy lane");
...
impl Session {
    ...
    async fn apply_settings_update_for_lane(
        ...
        // Ported from upstream alpha.10: environment configs refresh whenever
        // they changed, not only when selections were explicitly updated. This
        // is the shared body every settings-update path funnels through, so
        // upstream's inline copies of this logic land here.
