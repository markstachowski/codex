...
async fn server_model_match_uses_request_step_model() {
    ...
    session
        ...
        .expect("the server model matching the request step must be accepted");

    let error = session
        .enforce_server_model_match(step_context.as_ref(), nested_turn_model.as_str())
        .expect_err("a server model matching only the nested turn must be rejected");
    let message = error.to_string();
    assert!(message.contains("server-reported model"), "{message}");
    assert!(
...
async fn session_configuration_apply_runs_environment_validation_before_locked_inference() {
    ...
    let error = configuration
        ...
        .err()
        .expect("valid environments must expose the invalid locked inference update");
    assert!(matches!(
        error,
        ConstraintError::InvalidValue {
